...
def start_application(args):
    if args.option == "c":
        ...
        if args.ip:
            ...
            detect_live_hosts(args.ip, args.i)
        else:
            print_error("Para detecção de hosts vivos, forneça um endereço IP usando --ip")
            sys.exit(1)

    else:
        print_error("Opção inválida. Use 'c' para captura ou 'lh' para detecção de hosts vivos.")
        sys.exit(1)
...
{Colors.BRIGHT_YELLOW}⚠️  AVISO: Use apenas em redes que você possui ou tem permissão explícita para testar!{Colors.RESET}
    ...
    parser.add_argument("option", choices=["c", "lh"], 
                       help="Modo de operação: 'c' = Captura, 'lh' = Detecção de Hosts Vivos")
    ...
    parser.add_argument("--ip", help="Endereço IP para detecção de hosts vivos", required=False)
