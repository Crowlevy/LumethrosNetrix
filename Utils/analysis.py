...
def extract_packet_info(packet):
    ...
    if TCP in packet:
        ...
        packet_info['protocol_name'] = 'Desconhecido'
...
def extract_payload_data(packet):
    ...
    return payload_data[:100] + '...' if len(payload_data) > 100 else payload_data
...
def start_packet_analysis(interface, packet_count=10):
    print(f"Iniciando captura de pacotes e analise na interface {interface}...")
    sniff(iface=interface, prn=analyze_capture, count=packet_count, store=0)
    print("Captura de pacotes e analise completa.")
